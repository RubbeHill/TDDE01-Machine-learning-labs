...
This assignment is about classifying mails as spam or not spam. A data file containing 2470 e-mails manually classified as spam or regular e-mail was given. The file also contained information about the frequencies of different words in the e-mails. 
...
The results clearly show that this model isn't very reliable. Almost all e-mails were predicted to be not spam. Only a total of 7 e-mails in each data set are predicted as spam with a probability > 90%, and for the test set, 6 out of those 7 are false-positives. From this we can gather that the logistic regression model works ok when using a lower classifying principle, but is never really reliable because most predictions are lower than 0.9, and those that are over 0.9 are mostly wrong on the test data set. 
...
train_dfr = data.frame(x = train$Moisture, y = train$Protein)
test_dfr = data.frame(x = test$Moisture, y = test$Protein)

# Sort dataframe on x (needed for nice plot)
train_dfr = train_dfr[with(train_dfr, order(x)),]
test_dfr = test_dfr[with(test_dfr, order(x)),]
...
for(i in 1:6) {
  fit = lm(formula = train_dfr[,2] ~ poly(train_dfr[,1], i))
  y_hat_train = predict(fit, train_dfr)
  y_hat_test = predict(fit, test_dfr)
  
  mse_train = sum((train_dfr[,2] - y_hat_train)^2)/length(y_hat_train)
  train_mses = c(train_mses, mse_train)
  mse_test = sum((test_dfr[,2] - y_hat_test)^2)/length(y_hat_test)
  test_mses = c(test_mses, mse_test)
}

plot(seq(1:6), train_mses, type = 'l', ylim = c(3, 4.2), col="blue")
lines(seq(1:6), test_mses, col="red")
```
...
```{r 4_7, echo=FALSE}
cv_lasso = cv.glmnet(as.matrix(covariates), response, alpha = 1, family = "gaussian", lambda = seq(0, 0.4, 0.01))
plot(cv_lasso)
```